const t = import.meta.env.WEB_NAME || "WManage";
const v = import.meta.env.WEB_VISITION || "1.0.0";
...
export default {
  ...
  // 获取数据
  get(key: string) {
    const value: any = localStorage.getItem(`${t}-${v}-` + key);
    try {
      // 尝试解析 JSON 字符串
  ...
  clear() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const storageKey: any = localStorage.key(i);
  ...
  // 存储数据
  sysSet(value: any) {
    let storedData: any = localStorage.getItem(`${t}-${v}-sys`);
  ...
  // 获取数据
  sysGet() {
    const value: any = localStorage.getItem(`${t}-${v}-sys`);
    try {
      // 尝试解析 JSON 字符串
      return JSON.parse(value);
    } catch (error) {
